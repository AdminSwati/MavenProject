...
public class CITBank_CDCalculator_DataDrivenTesting {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		WebElement inideposit = driver.findElement(By.xpath("//input[@id='mat-input-0']"));		//Initial deposit amount
		WebElement length = driver.findElement(By.xpath("//input[@id='mat-input-1']"));			//length of CD in months
		...
		String path="C:\\Users\\swati\\eclipse-workspace\\SeleniumProject\\src\\test\\java\\Day100\\caldata2.xlsx";   // giving path to file to start reading
		int rows=XLUtility.getRowCount(path, "Sheet1");				// get a row count from the sheet
		System.out.println("row count is : " + rows);
		
		for(int i=1;i<=rows;i++)
		{			
			//Reading data from excel
			String inidepo= XLUtility.getCellData(path,"Sheet1",i,0);				//file,sheet,row #, col zero for initial deposit
			String interestrate=XLUtility.getCellData(path,"Sheet1",i, 1);		//interest rate
			String monthlength=XLUtility.getCellData(path,"Sheet1",i, 2);			//length
			String compoundingmonths=XLUtility.getCellData(path,"Sheet1",i, 3);	//compounding
			String exptotal=XLUtility.getCellData(path,"Sheet1",i, 4);	//expected total , will be compared with actual total
			...
			if(exptotal.equals(acttotal)) {			//if expected total = actual total then			
				...
				XLUtility.setCellData(path, "Sheet1",i, 6,"Passed");	//setting passed in 6th column (index start with zero)
				XLUtility.fillGreenColor(path, "Sheet1",i, 6);	//filling the color in 6th column if passed then greeen or faile then red.
			}
			else
			{
				XLUtility.setCellData(path, "Sheet1",i, 6,"Failed");
				XLUtility.fillRedColor(path, "Sheet1",i, 6);
			}
		}
